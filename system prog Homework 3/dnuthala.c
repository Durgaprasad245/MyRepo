...
void print_File(char *fileName, struct stat *stats, int *count, int tabSpaces) {
    if (!flags.is1) {
        ...
        if ((stats->st_mode & S_IFMT) == S_IFDIR) {
            printf("%*s[%d] %s %ld Bytes, %o, %s\n", 4 * tabSpaces, "", *count, fileName, (long) stats->st_size, stats->st_mode & 0777, ctime(&stats->st_mtime));
        } else {
            printf("%*s[%d] %s %ld Bytes, %o, %s\n", 4 * tabSpaces, "", *count, fileName, (long) stats->st_size, stats->st_mode & 0777, ctime(&stats->st_mtime));
        }
    }
...
void execute_command(char *command, char *fileName) {
    ...
    pid = fork();
    if (pid == 0) { 
        char **args = parse(command, fileName);
        execvp(args[0], args);
        perror("execvp"); 
        exit(EXIT_FAILURE);
    } else if (pid < 0) { 
...
void traverse_Directory(char *path, int tabSpaces, void (*funcPtr)(char *, struct stat *, int *, int)) {
    ...
    while ((dirent = readdir(parentDir)) != NULL) {
        ...
        if (check_File(dirent->d_name, &stats, tabSpaces)) {
            if (flags.is_e && flags.e_command) {
                execute_command(flags.e_command, filePath);
            } else if (flags.is_E && flags.E_command) {
                insertArray(files, *dirent->d_name);
            } else {
    ...
    if (flags.is_E && flags.E_command) {
        char **args = parse(flags.E_command, NULL);
        args[files.used] = NULL;
        execvp(args[0], args);
        perror("execvp");
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (!flags.is1 && (strcmp(argv[i], "-v") == 0)) {
            ...
            flags.is4 = TRUE;
            flags.arg4 = argv[i + 1];
        } else if (!flags.is_e && (strcmp(argv[i], "-e") == 0)) {
            flags.is_e = TRUE;
            flags.e_command = argv[i + 1];
        } else if (!flags.is_E && (strcmp(argv[i], "-E") == 0)) {
            flags.is_E = TRUE;
            flags.E_command = argv[i + 1];
        }
